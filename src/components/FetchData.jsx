...
const FetchData = (props) => {
  ...
  if (data === null) {
    return (
      <div
        className={`font-bold text-xl cursive italic h-[10rem] flex items-center px-6 md:px-10 animate-pulse ${
          wordTheme ? "text-black" : "text-gray-200"
        }`}
      >
        Loading...
      </div>
  ...
  return (
    <div
      className={`grid md:grid-cols-2 h-fit italic cursive font-semibold text-xl px-6 md:px-10 pt-[4rem] transition-all duration-300 ${
        wordTheme ? "text-black" : "text-gray-200"
      }`}
    >
      <div>
        <h1
          className={`text-3xl italic cursive ${
            verbDefinitionFound ? "flex" : "hidden"
          } justify-between`}
        >
          <div>
            Word:
            <span className="pl-2 font-[500] text-3xl">{word}</span>{" "}
            <span className="text-[1rem] montserrat font-normal pl-2">
              {phonetic}
            </span>
          </div>
        ...
        <div
          className={`font-bold text-xl cursive italic h-[10rem] flex items-center px-6 md:px-10 ${
            wordTheme ? "text-black" : "text-gray-200"
          } ${verbDefinitionFound ? "hidden" : "block"}`}
        >
        ...
        <br />
        <h1
          className={`cursive text-5xl font-semibold ${
            verbDefinitionFound ? "block" : "hidden"
          }`}
        >
          Verb
        </h1>
        <h1
          className={`text-3xl italic cursive pt-12 w-full md:w-[60%] ${
            verbDefinitionFound ? "block" : "hidden"
          }`}
        >
          Definition:{" "}
      ...
      <div>
        <h1
          className={`text-3xl italic cursive opacity-0 ${
            nounDefinitionFound ? "block" : "hidden"
          }`}
        >
        ...
        <div
          className={`font-bold text-xl cursive italic h-[10rem] flex items-center px-6 md:px-10 ${
            wordTheme ? "text-black" : "text-gray-200"
          } ${nounDefinitionFound ? "hidden" : "block"}`}
        >
